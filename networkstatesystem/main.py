...
def initArgs():
	...
	parser.add_argument('-t', '--ifdetail', help='Detailed Interface Inventory', action='store_true')
	parser.add_argument('-r', '--routedetail', help='Detailed Routing Table', action='store_true')
	parser.add_argument('-a', '--allinfo', help='All Device Information', action='store_true')
	parser.add_argument('-b', '--format', help='Output Format ("csv", "json", or "text")')
...
##### Start Main Section ######
	...
	for ipaddress, community, datasource in host_list:
		...
		device_detail_list[ipaddress] = device(hostname, osversion, ipaddress, datasource, routingdata, interfacedata)
